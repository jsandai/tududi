...
async function importUserData(userId, backupData, options = { merge: true }) {
    ...
    try {
        ...
        for (const relation of d.task_relations || []) {
            const resolvedSourceId = await resolve(
                Task,
                relation.source_task_uid,
                null,
                uidMaps.tasks
            );
            const resolvedTargetId = await resolve(
                Task,
            ...
            // A restore can map a pair onto ids in the opposite order, so
            // the stored orientation is decided here rather than carried over
            // from the backup.
            const [sourceId, targetId] = relationService.canonicalEndpoints(
            ...
            if (relation.relation_type === TaskRelation.TYPE.BLOCKS) {
                await relationService.lockBlockingGraph(transaction);
            }
