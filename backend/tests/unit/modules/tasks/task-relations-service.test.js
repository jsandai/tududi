...
const {
    filterTasksByParams,
} = require('../../../../modules/tasks/queries/query-builders');
const {
    serializeTasks,
...
describe('Task relation service', () => {
    ...
    it('excludes blocked tasks from actionable task queries', async () => {
        const blocker = await createTask('Blocker');
        const blocked = await createTask('Blocked');
        const available = await createTask('Available');
        await relationService.createRelation(
        ...
        expect(tasks.map((task) => task.id)).not.toContain(blocked.id);
        expect(tasks.map((task) => task.id)).toContain(available.id);
    });
