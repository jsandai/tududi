...
const { Project, Task, TaskRelation, sequelize } = require('../../models');
...
describe('Task relations across copies and project deletion', () => {
    ...
    it('keeps a symmetric relation canonical when the copy reorders its ids', async () => {
        ...
        // A copy can map the pair onto ids in the opposite order, which is
        // what the nested subtask pass does when it creates a parent's
        // children before moving on.
        const lower = await createTask('Lower id');
        const higher = await createTask('Higher id');
        await relationService.copyRelationsForTaskIds({
            [first.id]: higher.id,
            [second.id]: lower.id,
        });

        // Relating the same pair again must find the copied row rather than
        // storing a second one facing the other way.
        await expect(
        ...
        const pair = await TaskRelation.findAll({
            where: {
                [Op.or]: [
                    { source_task_id: lower.id, target_task_id: higher.id },
                    { source_task_id: higher.id, target_task_id: lower.id },
                ],
            },
        });
    ...
    it('clears relations on a bulk delete even with foreign keys off', async () => {
        const blocker = await createTask('Bulk blocker');
        const blocked = await createTask('Bulk blocked');
        await relationService.createRelation(
            user.id,
            blocker.uid,
            blocked.uid,
        ...
        // The delete paths switch foreign key enforcement off, so cleanup
        // must not depend on the cascade firing.
