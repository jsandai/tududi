...
// A related_to relation is symmetric but stored in one row, so its endpoints
// are always kept in id order. Every writer has to agree on that, or the same
// pair can be stored twice facing opposite ways.
// Finds the row that already represents this pair, if any. "A duplicates B"
// and "B duplicates A" describe the same pair, so a duplicates relation is
// matched in either direction. related_to is stored in canonical order, so
// matching it directionally is enough, and blocks is genuinely directional.
// Stable 32-bit key, following the convention in services/jobLock.js.
const BLOCKING_GRAPH_LOCK_KEY = crypto
    .createHash('sha256')
    .update('task_relations:blocking_graph')
    .digest()
    .readInt32BE(0);

// A cycle check reads the graph and then writes an edge, and the row locks
// taken on the two endpoints only serialise writers whose edges share an
// endpoint. Two writers adding edges that touch nothing in common can
// therefore each see no path back and still close a loop between them. One
// lock for the whole blocking graph keeps the check and the write atomic
// against every other blocking write. PostgreSQL only: SQLite has a single
// writer already.
async function lockBlockingGraph(transaction) {
    if (!isPostgres()) return;
    await sequelize.query('SELECT pg_advisory_xact_lock(:key)', {
        replacements: { key: BLOCKING_GRAPH_LOCK_KEY },
        transaction,
    });
}

// The orientation a pair is stored in, for any writer. related_to is
// symmetric but stored in one row, so its endpoints always go in id order;
// the other types keep the direction they were given.
function canonicalEndpoints(relationType, sourceId, targetId) {
    if (relationType !== TaskRelation.TYPE.RELATED_TO) {
        return [sourceId, targetId];
    }
    return canonicalPair(sourceId, targetId);
}

function relationPairScope(relationType, sourceId, targetId) {
    if (relationType !== TaskRelation.TYPE.DUPLICATES) {
        return { source_task_id: sourceId, target_task_id: targetId };
    }
    return {
        [Op.or]: [
            { source_task_id: sourceId, target_task_id: targetId },
            { source_task_id: targetId, target_task_id: sourceId },
        ],
    };
}
...
function canonicalize(currentTask, relatedTask, inputType) {
    ...
    if (inputType === 'related_to') {
        const [firstId] = canonicalPair(currentTask.id, relatedTask.id);
        const [sourceTask, targetTask] =
            firstId === currentTask.id
                ? [currentTask, relatedTask]
                : [relatedTask, currentTask];
        return {
            sourceTask,
            targetTask,
...
async function createRelation(
    ...
    return await sequelize.transaction(
        ...
        async (transaction) => {
            ...
            const pairScope = relationPairScope(
                relationType,
                sourceTask.id,
                targetTask.id
            );
            ...
            if (relationType === TaskRelation.TYPE.BLOCKS) {
                await lockBlockingGraph(transaction);
            }
...
async function copyRelationsForTaskIds(taskIdMap, transaction = null) {
    ...
    return await TaskRelation.bulkCreate(
        relations.map((relation) => {
            // New ids need not keep the old order, so a symmetric relation is
            // put back into canonical order rather than copied as it stood.
            const [source_task_id, target_task_id] = canonicalEndpoints(
                relation.relation_type,
                taskIdMap[relation.source_task_id],
                taskIdMap[relation.target_task_id]
            );
            return {
                source_task_id,
                target_task_id,
